...
--- All credit for majority of this code goes to the Tokyonight theme
--- its been used as a basis for the theme.
...
function M.mod(modname)
	...
	-- print("looking for:" .. me .. "/" .. modname:gsub("%.", "/") .. ".lua")
...
---@param foreground string fg colour
---@param background string bg colour
...
function M.invert(color)
	...
	elseif type(color) == "string" then
		local hsluv = require("fallout4_improved.hsluv")
		if color ~= "NONE" then
...
---@param groups fallout4_improved.Highlights
...
function M.cache.file(key)
	return vim.fn.stdpath("cache") .. "/fallout4_improved-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
	---@type boolean, fallout4_improved.Cache
...
function M.cache.clear()
	local pattern = vim.fn.stdpath("cache") .. "/fallout4_improved-*.json"
