...
function M.setup(opts)
	...
	local colors = require("fallout4_improved.colors").setup(opts)
	local groups = require("fallout4_improved.groups").setup(colors, opts)

	-- only needed to clearn when no the default colorscheme
	if vim.g.colors_name then
		vim.cmd("hi clear")
	end

	vim.o.termguicolors = true
	vim.g.colors_name = "fallout4_improved-" .. opts.style
